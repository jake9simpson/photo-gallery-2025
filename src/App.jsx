...
const gallery = Object.keys(fullResModules)
  ...
  .map((path) => {
    ...
    return {
      ...
      fileName: fileName // Keep filename for download
    };
  });

// --- ICONS (Added Download Icon) ---
const Icons = {
  ...
  Download: () => <svg className="meta-icon" fill="currentColor" viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>,
...
function App() {
  ...
  const loadMetadata = async (src) => {
    try {
      setMetadata(null); // Reset metadata while loading
      ...
      // Even if metadata fails, ensure we set empty object so Download button still shows
      setMetadata({});
    }
  };
  ...
  return (
    <div>
      ...
      >
        <h1>Jake's Best Shots 2025</h1>
        <p className="subtitle">High Resolution Portfolio • {gallery.length} Shots</p>
      </motion.header>
          ...
          >
            ...
            {metadata && (
              ...
              >
                ...
                {metadata.date && <div className="meta-item"><Icons.Calendar /> <span>{metadata.date}</span></div>}
                
                {/* DOWNLOAD BUTTON */}
                <a 
                  href={gallery[selectedIndex].full} 
                  download={gallery[selectedIndex].fileName}
                  className="meta-item" 
                  style={{ 
